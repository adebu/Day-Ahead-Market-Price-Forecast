...
def load_esios_data(start_date, end_date, training_data):
    ...
    while current_date <= end_date:
        ...
        try:
            generation_data = ESIOS.get_training_data(current_date_formated, query_year_formated, training_data)
            df = ESIOS.extract_values(generation_data, training_data)

            ESIOS.connection_to_DB(df, training_data, current_date_formated)
        except Exception as e:
            print(f'Values were not found for {training_data} for the month{current_date_formated}')
            print(e)
